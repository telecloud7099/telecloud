...
| 2026-07-26 | `8efd733a4f04233e9fbfcf3a0efda69a17549b2b` | 1 PASS, 2 PASS, 3 PASS, 4 PASS, 5 PASS, 6 PASS | 7 PASS, 8 PASS | Item 2: Cloud Password enabled, verified (not N/A). Run against `http://127.0.0.1:8080` directly (the VM's self-contained app — the same path Cloudflare Tunnel will later expose). Initial run showed 22 console errors, all `404` on `/thumbnail/{id}` for accumulated test-artifact files from Phase 15 testing (not a regression) — root-caused and resolved via a controlled cleanup: 6 confirmed test files deleted through the app's own `DELETE /files` API (7 Telegram messages removed — one file was a 2-chunk upload), VM-side scratch files removed, upload_sessions confirmed clean (0 orphaned `uploading` rows). Re-verified after cleanup: console clean, fresh upload/thumbnail/download cycle confirmed working with no regressions. Items 7-8 (`phase9_functional_check.sh`): Range request returned `206`/correct `Content-Range`/exact 100-byte body; thumbnail hash identical before/after a normal `telecloud-app` restart — 4/4 automated checks passed. |
