...
| 2026-07-23 | 5 + 6 (full VM reboot, per-service recovery) | **PASS** | `sudo reboot` from the guest; all three containers (`telecloud-app`, `telecloud-nginx`, `telecloud-postgres`) came back automatically with zero manual intervention — Docker daemon started on boot, `restart: unless-stopped` brought every service up, `telecloud-app`/`telecloud-postgres` both reached `healthy`, nginx reachable. Confirms both the whole-stack recovery (Scenario 5) and per-service recovery (Scenario 6) in one pass. |
| 2026-07-23 | 7 (in-flight upload survives full reboot) | **PASS** | Session `44563219-9a4c-47da-9783-b93eb464a076`, rebooted at 23.6MB/500MB in. Post-reboot: `session_status` stayed `uploading`, `part_progress: null`, `next_part_number` unchanged — identical recovery invariant as Scenario 1, now proven across the harsher full-reboot path. Two organic, unrelated failures occurred during the retry attempts before final success: (1) a post-boot network-settling window where both Neon Postgres and Telethon's MTProto connection hit transient `Connection refused` right after boot, caught cleanly as `phase: "failed"` with a retryable error; (2) an unrelated **upstream Telethon v1.32.1 bug** (`telethon.client.updates`'s own admitted bug, `RuntimeError: Should not be applying the difference when neither account or secret was diff was active`) crashed the shared client's update-handling loop and disconnected it mid-upload, again caught cleanly as `"Cannot send requests while disconnected"` with no corruption. Both are genuinely different root causes, and the app's failure-handling contract (mark failed, surface a retryable error, never corrupt state) held correctly both times — arguably stronger evidence for this scenario's stated goal ("in-flight uploads fail gracefully and can be resumed/restarted") than a single clean success would have been. Final successful upload verified: downloaded and sha256-compared against the original — **`a54a5bf60e73e2d73e85f2754db1526dac580b24b5ddfe9fd4387dcbb41d92b7` on both sides, exact match.** |
| 2026-07-23 | 8 (post-test integrity checks) | **PASS** | **Database consistency:** every test session either completed with a matching confirmed-chunk count or was explicitly aborted; `GET /uploads` confirmed empty (no lingering sessions) after cleanup. **Telegram message consistency (the one genuinely uncertain item):** verified by direct observation, not inference — `phase11_orphan_check.py` searched Saved Messages for the docker-kill test's caption `group_id` (session `36f0548f-...`, killed at only 8% through) and found **zero matching messages**, confirming no orphan was created (consistent with the reasoning that `send_file()` couldn't have completed that early, but now empirically closed rather than assumed). **File verification:** both final Scenario 1 and Scenario 7 uploads sha256-matched their originals (already logged above). **Health endpoints:** `docker compose ps` confirmed all three containers healthy throughout. **Logs:** reviewed across the full test window; every anomaly found was independently root-caused and documented above (stale-client contention, event-loop-blocking correlation, docker-kill-vs-crash distinction, post-boot network settling, the Telethon update-handler bug) rather than left unexplained. **Cleanup:** aborted the leftover kill-test session, deleted all 3 phase-11 test files (`38d67f5f`, `ebe995ad`, `b6c24d4d`) from the real file listing — `{"status":"success","deleted":3,"failed":[]}`. |
